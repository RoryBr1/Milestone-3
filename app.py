...
def get_recipes():
    ...
    # Find all categories in the collection, "categories" (used for the sort-by-category dropdown):
...
# Admin login authentication (taken from CodeInstitute Flask-MongoDB mini-project):
...
def add_recipe():
    # Create recipe object by requesting form data and assigning it in key-value pairs
    if request.method == "POST":
        recipe = {
...
def edit_recipe(recipe_id):
    if request.method == "POST":
        recipe_name = request.form.get("recipe_name").title()
        # Retrieve the edited data from the form on edit-recipe page, and assign it to the submit object:
        submit = {
            "recipe_name": request.form.get("recipe_name").title(),
            "prep_time": request.form.get("prep_time"),
            "category_name": request.form.get("category_name"),
        ...
        # Update the object with relevant ObjectId in the "recipes" collection, with the new data in the "submit" object:
...
def add_category():
    # Request data from the form on show-categories page, and assign to "category" variable:
    if request.method == "POST":
        category = {
    ...
    categories = mongo.db.categories.find()
    return redirect(url_for("show_categories"), categories=categories)
